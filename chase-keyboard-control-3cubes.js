...
function chase(destinationX, destinationY, sourceX, sourceY, sourceAngle) {
  const diffX = destinationX - sourceX
  const diffY = destinationY - sourceY
  const distance = Math.sqrt(diffX * diffX + diffY * diffY)
  if (distance < 50) {
    return [0, 0] // stop
  }

  let relAngle = (Math.atan2(diffY, diffX) * 180) / Math.PI - sourceAngle
...
async function main() {
  // start a scanner to find nearest three cubes
  const cubes = await new NearScanner(3).start()

  // connect three cubes (tom chases jerry, and spike chases tom)
  ...
  let spikeY = 0
  let spikeAngle = 0
  spike.turnOnLight({ durationMs: 0, red: 255, green: 255, blue: 0 })
  ...
  setInterval(() => {
    spike.move(...chase(tomX, tomY, spikeX, spikeY, spikeAngle), 100)
  }, 50)
